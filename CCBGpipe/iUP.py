...
class UniprotDownloader:
    def __init__(self, min_gene_length, max_gene_length, uniprot_search, header_regex, header_regex_ignorecase=True):
    ...
    def _download_from_uniprot(self, outfile):
        comm='wget -O ' + outfile + ' "' + self._get_uniprot_url() + '"'
        print (comm)
        pyfastaq.utils.syscall(comm)
...
if __name__=='__main__':
    argv=sys.argv
    min_gene_length=int(argv[1])
    max_gene_length=int(argv[2])
    uniprot_search=argv[3]
    header_regex=argv[4]
    outprefix=argv[5]
    downloader=UniprotDownloader(min_gene_length,max_gene_length,uniprot_search,header_regex,outprefix)
    downloader.run(outprefix)
